    ...
    egui::TopBottomPanel::top("menu_bar").show(ctx, |ui| {
        egui::menu::bar(ui, |ui| {
            ui.menu_button("Session", |ui| {
                if ui.add_enabled(app.can_connect(), egui::Button::new("New Connection...")).clicked() {}
                if ui.add_enabled(app.can_disconnect(), egui::Button::new("Disconnect")).clicked() {
                    app.disconnect();
                }
                ui.separator();
            ...
            if app.can_disconnect() {
                ...
                if ui.button(if app.show_sidebar { "◀ Hide" } else { "▶ Show" }).clicked() {
...
fn render_connection_form(app: &mut RuttyApp, ui: &mut egui::Ui, compact: bool) {
    ...
    if !app.saved_sessions.is_empty() {
        ui.horizontal(|ui| {
            ...
            ui.add_enabled_ui(!app.selected_session.is_empty(), |ui| {
                if ui.button("🗑").on_hover_text("Delete saved session").clicked() {
    ...
    ui.add_enabled_ui(enabled, |ui| {
        ui.horizontal(|ui| {
            ui.label("Host:");
            ui.add_sized([available - 40.0, 20.0], egui::TextEdit::singleline(&mut app.form.host).hint_text("hostname or IP"));
        });
    });
    ui.add_space(4.0);
    ui.add_enabled_ui(enabled, |ui| {
        ui.horizontal(|ui| {
            ui.label("Port:");
            ui.add_sized([80.0, 20.0], egui::TextEdit::singleline(&mut app.form.port).hint_text("22"));
        });
    });
    ui.add_space(4.0);
    ui.add_enabled_ui(enabled, |ui| {
        ui.horizontal(|ui| {
            ui.label("User:");
            ui.add_sized([available - 40.0, 20.0], egui::TextEdit::singleline(&mut app.form.username).hint_text("username"));
        });
    });
    ui.add_space(4.0);
    ui.add_enabled_ui(enabled, |ui| {
        ui.horizontal(|ui| {
            ui.label("Pass:");
            ui.add_sized([available - 40.0, 20.0], egui::TextEdit::singleline(&mut app.form.password).password(true).hint_text("password (or use key)"));
        });
    });
    ui.add_space(4.0);
    ui.add_enabled_ui(enabled, |ui| {
        ui.horizontal(|ui| {
            ui.label("Key:");
            ui.add_sized([available - 40.0, 20.0], egui::TextEdit::singleline(&mut app.form.key_file).hint_text("path to private key (optional)"));
        });
    });
    ui.add_space(8.0);
...
fn render_terminal(app: &mut RuttyApp, ui: &mut egui::Ui) {
    let session = match app.session.as_ref() {
        Some(s) => s,
        None => {
            ui.centered_and_justified(|ui| { ui.label("Not connected."); });
            return;
        }
    };
    ...
    let do_scroll = app.needs_scroll && app.auto_scroll;
    app.needs_scroll = false;

    // Also scroll when user presses Enter (sending input = want to see output)
    let enter_pressed = ui.ctx().input(|i| i.key_pressed(egui::Key::Enter));

    ScrollArea::both()
        .auto_shrink([false, true])
        .show(ui, |ui| {
            if do_scroll || enter_pressed {
                // Scroll to cursor row minus some padding, so cursor isn't right at bottom
                let scroll_y = (cur_row as f32 - visible_rows as f32 * 0.2).max(0.0) * row_height;
                ui.scroll_to_rect(
                    egui::Rect::from_min_size(egui::Pos2::new(0.0, scroll_y), egui::Vec2::new(1.0, 1.0)),
                    Some(Align::Min),
                );
            }
            ...
            for row in 0..rows {
                ui.horizontal(|ui| {
                    ...
                    let mut run_color = Color32::from_rgb(200, 210, 200);

                    for col in 0..cols {
                        let (ch, fg) = if let Some(ref cell) = session.cell(row, col) {
                            let contents = cell.contents();
                            let ch = if contents.is_empty() { ' ' } else { contents.chars().next().unwrap_or(' ') };
                            (ch, vt100_fg_to_egui(cell.fgcolor()))
                        } else {
                        ...
                        if fg != run_color {
                            if !run_text.is_empty() {
                                ui.label(egui::RichText::new(std::mem::take(&mut run_text)).font(font_id.clone()).color(run_color));
                            }
                            run_color = fg;
                        }
                    ...
                    let trimmed = run_text.trim_end().to_string();
                    if !trimmed.is_empty() {
                        ui.label(egui::RichText::new(trimmed).font(font_id.clone()).color(run_color));
                    }
                });
            }
            ...
            while let Some(event) = i.events.pop() {
                match event {
                    egui::Event::Key { key, pressed: true, modifiers, .. } => {
                        ...
                        if !bytes.is_empty() { app.send_keys(bytes); }
                    }
...
fn key_to_ssh_bytes(key: Key, mods: Modifiers) -> Vec<u8> {
    if mods.ctrl {
        match key {
            Key::A => return vec![1], Key::B => return vec![2], Key::C => return vec![3],
            Key::D => return vec![4], Key::E => return vec![5], Key::F => return vec![6],
            Key::G => return vec![7], Key::H => return vec![8], Key::I => return vec![9],
            Key::J => return vec![10], Key::K => return vec![11], Key::L => return vec![12],
            Key::M => return vec![13], Key::N => return vec![14], Key::O => return vec![15],
            Key::P => return vec![16], Key::Q => return vec![17], Key::R => return vec![18],
            Key::S => return vec![19], Key::T => return vec![20], Key::U => return vec![21],
            Key::V => return vec![22], Key::W => return vec![23], Key::X => return vec![24],
            Key::Y => return vec![25], Key::Z => return vec![26],
            Key::OpenBracket => return vec![27], Key::Backslash => return vec![28],
            Key::CloseBracket => return vec![29], Key::Num6 => return vec![30],
            Key::Minus => return vec![31],
            _ => {}
        }
    }
    match key {
        Key::Enter => vec![b'\r'], Key::Tab => vec![b'\t'], Key::Backspace => vec![0x7f],
        Key::Escape => vec![0x1b],
        Key::ArrowUp => vec![0x1b, b'[', b'A'], Key::ArrowDown => vec![0x1b, b'[', b'B'],
        Key::ArrowRight => vec![0x1b, b'[', b'C'], Key::ArrowLeft => vec![0x1b, b'[', b'D'],
        Key::Home => vec![0x1b, b'[', b'H'], Key::End => vec![0x1b, b'[', b'F'],
        Key::PageUp => vec![0x1b, b'[', b'5', b'~'], Key::PageDown => vec![0x1b, b'[', b'6', b'~'],
        Key::Delete => vec![0x1b, b'[', b'3', b'~'], Key::Insert => vec![0x1b, b'[', b'2', b'~'],
        Key::F1 => vec![0x1b, b'O', b'P'], Key::F2 => vec![0x1b, b'O', b'Q'],
        Key::F3 => vec![0x1b, b'O', b'R'], Key::F4 => vec![0x1b, b'O', b'S'],
        Key::F5 => vec![0x1b, b'[', b'1', b'5', b'~'], Key::F6 => vec![0x1b, b'[', b'1', b'7', b'~'],
        Key::F7 => vec![0x1b, b'[', b'1', b'8', b'~'], Key::F8 => vec![0x1b, b'[', b'1', b'9', b'~'],
        Key::F9 => vec![0x1b, b'[', b'2', b'0', b'~'], Key::F10 => vec![0x1b, b'[', b'2', b'1', b'~'],
        Key::F11 => vec![0x1b, b'[', b'2', b'3', b'~'], Key::F12 => vec![0x1b, b'[', b'2', b'4', b'~'],
        _ => vec![],
    }
}
...
fn vt100_fg_to_egui(c: vt100::Color) -> Color32 {
    match c {
        ...
        vt100::Color::Idx(i) => match i {
            0 => Color32::BLACK, 1 => Color32::from_rgb(205, 0, 0),
            2 => Color32::from_rgb(0, 205, 0), 3 => Color32::from_rgb(205, 205, 0),
            4 => Color32::from_rgb(0, 0, 205), 5 => Color32::from_rgb(205, 0, 205),
            6 => Color32::from_rgb(0, 205, 205), 7 => Color32::from_rgb(229, 229, 229),
            8 => Color32::from_rgb(127, 127, 127), 9 => Color32::from_rgb(255, 0, 0),
            10 => Color32::from_rgb(0, 255, 0), 11 => Color32::from_rgb(255, 255, 0),
            12 => Color32::from_rgb(0, 0, 255), 13 => Color32::from_rgb(255, 0, 255),
            14 => Color32::from_rgb(0, 255, 255), 15 => Color32::from_rgb(255, 255, 255),
            n if n < 232 => { let n = n - 16; Color32::from_rgb((n/36)*51, ((n/6)%6)*51, (n%6)*51) }
            n => { let g = ((n - 232) * 10 + 8) as u8; Color32::from_rgb(g, g, g) }
        },
...
fn render_status_dot(ui: &mut egui::Ui, state: ConnState, time: f32) {
    ...
    if is_active {
        ...
        ui.painter().circle_filled(dot_center, size / 2.0 + 3.0, glow_color);
    }
}
